-- Keep the native MPJ5 shape gate global while requiring item-level realization
-- lineage only inside the current ko_zh request/refusal/thanks pack scope.
--
-- The Edge function intentionally omits item_lineage for uncovered speech acts and
-- zh_ko missions. The previous diagnostic gate nevertheless required the covered
-- lineage prompt version for every current native MPJ5, making valid uncovered
-- missions impossible to save.
...
BEGIN
  IF v_mission IS NULL
     OR v_mission->'provenance'->>'prompt_version'
        IS DISTINCT FROM 'mission_v5_mpj5_minidiscourse_v4_concise_learner_flow' THEN
    RETURN NEW;
  END IF;
