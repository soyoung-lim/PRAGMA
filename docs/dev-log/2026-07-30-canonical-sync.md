...
- 프롬프트 스냅샷 내용은 최신이었지만 provenance가
  `git_commit=44805b6`, `git_dirty=true`로 남아 있었다.
- v4 migration·Edge 배포·생성 저장 스모크·사람 reviewed는 여전히 미실행 상태다.

## 갱신한 것

- 세 정본의 상태일과 구현 기준선을 2026-07-30 현재 branch와 `95e8c3a`에 맞춤
...
- 프롬프트 스냅샷 12종을 clean HEAD `3b49a10`, `git_dirty=false`로 재생성
...
- 실행 코드가 바뀌지 않아 전체 테스트와 build는 다시 실행하지 않았고, `95e8c3a`의
