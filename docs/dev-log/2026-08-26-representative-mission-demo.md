...
- 상단 학습자·교수자 카드는 기존 1:1 grid 폭을 유지하고, 내용 길이가 달라도 내부 행동 버튼의
  하단선이 맞도록 정렬했다.
- `통합 구조 보기`와 `대표 미션 시연`은 동일한 190px 폭·흰색 아웃라인 스타일로 통일했다.
  대표 미션은 노란 재생 아이콘만 식별점으로 남겨 주 행동보다 강했던 검은 채움 CTA를 제거했다.
- `npm.cmd run typecheck`, `git diff --check`를 통과했다. 로컬 브라우저 한 화면에서 두 카드의
  동일한 경계·높이·버튼 하단선과 두 디펜스 CTA의 동일 폭·위계를 확인했다.
