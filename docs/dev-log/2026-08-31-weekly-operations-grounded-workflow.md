...
- 운영 화면 검토에서 애프리콧 면적이 폼을 탁하게 보이게 한다는 피드백을 반영했다. 헤더·본문을 화이트/쿨그레이로 되돌리고 PRAGMA 옐로는 식별선, 청량한 블루는 진행·필수 표시·선택·포커스, 네이비는 제목·주 CTA로 역할을 제한했다. 표적 2 tests와 typecheck가 통과했으며 입력 구조와 데이터 계약은 변경하지 않았다.
- 최종 색상 검토에서는 블루 혼합안을 철회하고 PRAGMA 네이비·옐로 정본으로 복귀했다. 기존의 간결한 제목·중간 폭 캔버스·2열 입력 흐름은 유지하면서 헤더와 CTA는 네이비, 브랜드 표시·진행선·선택 상태는 옐로, 본문은 웜 아이보리로만 조정했다.
- 후속 밀도 검토에서 팝업 프레임은 유지하되 실제 질문·입력·선택·하단 동작 영역을 중앙 `640px` 컬럼으로 제한했다. 옵션 카드가 캔버스 끝까지 불필요하게 늘어나지 않도록 하면서 기존 2열 읽기 순서와 작은 화면의 전체 폭 동작은 유지했다.
- 실제 운영 화면에서는 입력 영역만 줄여 프레임 안쪽 여백의 비율이 커지는 문제가 확인됐다. `640px` 입력 컬럼에 맞춰 팝업 최대 폭을 `720px`로 줄이고 헤더 패딩·브랜드 표시·제목 크기도 함께 축소해 프레임과 콘텐츠의 전체 균형을 맞췄다.
