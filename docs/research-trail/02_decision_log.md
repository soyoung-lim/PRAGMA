...
## DEC-20260815-10 · 권한 관리는 기존 운영 경로로, 권한검사 증거는 연구 현황에 보존

- 날짜: 2026-08-15
- 상태: 채택
- 문제: 학습자는 기존 `학습자 관리`에서 운영하고 교수자·외부 전문가는 소수이므로 별도 `사용자·권한` 준비 화면은 빈 메뉴만 늘린다. 그러나 관리자·전문가·학습자의 실제 접근 경계를 검사한 결과는 504개 생성 게이트와 심사 방어에 필요하다.
- 결정:
  - 사이드바 `설정 · 사용자·권한`과 빈 `/admin/users` 라우트를 삭제한다.
  - 계정 운영은 기존 학습자 관리·인증·전문가 등록 절차를 사용한다.
  - `/admin/research-qa`의 `세 사용자 역할의 실제 권한 검사` 항목에서 최신 불변 운영검증 행의 검사 시각, 규약, 코드 버전, 세 계정 분리, 역할별 접근 차단과 연구자료 무변경 결과를 펼쳐 확인한다.
  - 권한관리 UI의 존재를 연구 기여로 주장하지 않고, 서버 RLS 경계와 실제 계정 검사 증거만 품질 게이트로 사용한다.
- 관련 기록: `docs/dev-log/2026-08-15-permission-evidence-visibility.md`
- 관련 기록: `docs/dev-log/2026-08-15-bounded-external-validation.md`
- 관련 커밋: 확인 필요

## DEC-20260815-11 · 관리자 사이드바는 수업 전 제작·검증·편성 순서와 학기 후 연구자료 처리를 분리

- 날짜: 2026-08-15
- 상태: 채택. `DEC-20260815-07`의 다섯 단계 설명 가운데 수행기록 내려받기의 단계 분류를 대체한다.
- 문제:
  - 그룹 번호 0~4와 품질 절차 번호 1~5가 함께 보여 어떤 번호가 실제 작업 순서인지 불명확했다.
  - 같은 제작 흐름인 개별·배치 생성이 별도 메뉴였고, 생성과 품질 확인이 떨어져 있었다.
  - 학기 종료 후 수행하는 연구자료 내려받기가 수업 전 공개 절차에 포함되어 9월 준비를 끝내도 미완성처럼 보였다.
  - `/admin/review`는 실제 승인 동작이 없는 안내 화면이었고, 학습 미션 조립의 내부 확인 및 최종 학습자 공개와 역할이 겹쳐 보였다.
- 검토한 대안:
  - 학습자·수업 운영·연구/검수를 세 개의 독립 관리자 화면으로 분할: 개념적으로는 전체 시스템 구조의 세 축과 맞지만, 생성·조립·검증·편성을 오가는 관리자에게 다음 위치를 다시 찾게 하므로 현재 단계에서는 기각.
  - 학습자 화면은 기존처럼 분리하고, 관리자 사이드바는 실제 작업 순서를 한 줄로 유지: 채택.
- 결정:
  - 관리자 사이드바 그룹은 `재료와 규칙 → 문항 생성 → 품질 검증과 공개 → 수업 편성 → 학습자와 연구자료` 순서로 둔다. 그룹 번호는 없애고 수업 전 품질 절차에만 1~4단계를 표시한다.
  - 개별 생성과 배치 생성은 사이드바의 `문항 생성` 하나로 묶고 동일 화면군의 두 탭으로 제공한다.
  - 품질 절차는 연구자 기준답안 판정, 외부 전문가 표본 확인, 504개 자동 결과 확인·경고 검토, 교수자의 학습자 사용 승인 네 단계다.
  - 수행기록 내려받기는 DB 공개 게이트를 변경하지 않고 `학습자와 연구자료`로 옮긴다. 이는 학기 종료 후 연구자료 처리이며 수업 전 4단계의 완료 여부에 포함하지 않는다.
  - `/admin/review` 안내 화면은 제거하고 기존 주소는 `학습 미션 조립`으로 이동시킨다. 미션 조립의 `reviewed` 전환은 내부 확인이고, 정식 504개 학습자 공개는 4단계 교수자 승인만 권위 있는 최종 결정이다.
  - 시나리오 아카이브는 제작·검증 순서 밖의 참조 영역으로 분리한다.
- 주장 경계:
  - 이번 변경은 정보 구조와 책임 경계를 명확히 한 것이며, DB 게이트·콘텐츠 품질·연구 결과를 새로 검증했다는 뜻이 아니다.
- 관련 기록: `docs/dev-log/2026-08-15-admin-workflow-navigation.md`
- 관련 커밋: 확인 필요

## DEC-20260815-12 · 설명 화면은 비율 정본을 만들지 않고 실제 생성·운영 계약을 공유

- 날짜: 2026-08-15
- 상태: 채택
- 문제:
  - 수준별 문항 화면이 실제 미션 생성 경로에서 쓰지 않는 `MPJ 80%·30%·10%`, 후보 수 `3·5·7`, 직접성 단일축을 현행 정책처럼 제시했다.
  - 전체 시스템 구조 화면에 폐기된 사전·중간·사후 평가와 이중 평정이 남아 심사에서 수행하지 않는 연구 결과를 요구받을 위험이 있었다.
- 검토한 대안:
  - 문항 화면을 삭제: 잘못된 설명은 없어지지만 실제 수준 정책을 심사위원에게 보여줄 근거 화면도 사라져 기각.
  - 기존 화면을 유지하고 세 문장만 수정: 화면의 핵심 비율과 문항 유형 자체가 현행 미션 스키마와 달라 기각.
  - 실제 생성 코드와 수준 정책을 공유하는 설명 화면으로 재작성하고 전체 구조를 현행 4단계 품질 절차에 맞춤: 채택.
- 결정:
  - 모든 수준은 `MPJ 5개 + 번역 또는 통역 산출 1개` 구조를 공유하고, 수준 차이는 문장 복잡도·표현 자원 조합·원문 길이로 설명한다.
  - 수준 정책은 공통 코드 정본을 실제 미션 생성과 설명 화면이 함께 사용한다.
  - 후보는 화행별 목표 화용 요소를 기준으로 만들고, 판정은 각 요소의 과소·적정·과잉 세 대역을 사용한다. 모든 화행을 직접성 단일축으로 일반화하지 않는다.
  - 기준답안 A·B·C는 시스템 판정 장치의 교정 사례이며, A·B·C 위치나 후보 세 개를 학습 수준별 고정 정책으로 해석하지 않는다.
  - `/architecture`는 사전·중간·사후 및 이중 평정을 제거하고 `시스템 조건 확인 + 교수자 최종 학습자 사용 승인` 책임 경계와 현행 학습·연구자료 흐름을 제시한다.
- 주장 경계:
  - 기존 화면 외에서 `80%·30%·10%` 비율을 확정 정본으로 뒷받침하는 근거가 확인되지 않았으므로 논문 근거로 사용하지 않는다.
  - 화면 정렬은 구현·설계 설명의 일관성 개선이며 학습효과나 품질 검증 결과가 아니다.
- 관련 기록: `docs/dev-log/2026-08-15-static-design-screen-alignment.md`
- 관련 커밋: 확인 필요
