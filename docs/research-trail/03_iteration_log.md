...
## ITER-20260815-07 · 기술 중심 연구 메뉴에서 이해 가능한 품질관리 업무 흐름으로

- 날짜: 2026-08-15
- 시작 문제:
  - 화면을 본 사용자가 각 용어의 뜻, 행위자, 검토 대상, 검증과 검수의 차이, 공개 위치를 연속해서 질문해야 했다.
  - 전문가 화면이 DB·프로토콜 구조를 앞세워 실제로 무엇을 어떤 순서로 판단할지 파악하기 어려웠다.
- 변경:
  - 메뉴와 화면을 다섯 단계의 실제 산출물·행동 중심으로 재구성하고 공통 용어 안내를 추가했다.
  - Gold와 AI 학습문항 검토를 별도 단계로 유지하되 같은 외부 전문가 2인을 재사용할 수 있다는 점과 기록 분리 원칙을 명시했다.
  - 전문가 화면을 상황·의미, 중국어 표현, 종합 판단의 순서로 바꾸고 내부 ID는 쉬운 설명 뒤로 내렸다.
  - 개선 기능을 학습 분석 영역으로 이동하고 `학습 콘텐츠 개선`으로 다시 명명했다.
- 검증 결과:
  - localhost에서 연구 현황, 관리자 검토 운영, 전문가 작업, 공개, 개선 화면의 DOM과 메뉴 배치를 확인했다.
  - typecheck, 169개 테스트와 1,916-module production build가 통과했다.
  - DB schema·원격 데이터·실제 전문가 판정은 변경하거나 생성하지 않았다.
- 예상과 달랐던 점:
  - 첫 번째 쉬운 말 치환만으로는 `누가 무엇을 왜 하는가`라는 질문이 사라지지 않았다. 메뉴의 분류와 화면 위계를 함께 바꿔야 했다.
- 다음 설계에 반영할 교훈:
  - 용어 설명을 읽기 전에는 메뉴 목적을 알 수 없다면 메뉴명 자체가 잘못된 것이다.
  - 연구용 내부 식별자와 상태명은 운영 화면에서 필요한 경우에만 고급 정보로 제공한다.
- 관련 Decision / Evidence: `DEC-20260815-07`, `EVD-20260815-07`

## ITER-20260815-08 · 외부 전문가 60분 상한과 연구 책임자 전수 검토로 재설계

- 날짜: 2026-08-15
- 시작 문제: 외부 전문가 2인×504개 전수 판정은 현실적인 섭외 범위를 크게 넘었다.
- 변경: 외부 검토를 9화행 층화 Gold 18건으로 제한하고, 504개는 자동 점검과 연구 책임자 전수 판정으로 분리했다. 전용 관리자 작업대와 원자적 공개 조건을 연결했다.
- 검증 결과: typecheck, 전체 172개 테스트와 production build가 통과했고 migration dry-run 뒤 원격 DB에 적용했다. 실제 전문가·연구자 판정 행은 만들지 않았다.
- 다음 확인: 실제 5건 예비 판정으로 45~60분 가정을 확인하고, 초과 시 입력 위계를 간소화한다.
- 관련 Decision / Evidence: `DEC-20260815-08`, `EVD-20260815-08`

## ITER-20260815-09 · 사전 표본추출·확전 규칙과 증거 경계 분리

- 날짜: 2026-08-15
- 시작 문제: 외부 18건의 추출 주체·시점·방법이 고정되지 않았고, 문제 발견 뒤의 조치와 30·18·504 각 근거의 주장 범위가 분리되지 않았다.
- 변경:
  - 서버가 정식 생성 lock 전에 연구자 승인 Gold 모집단을 고정하고 화행별 최초 2건·예비 사례를 결정론적 무작위 순서로 보존하는 표본계획을 추가했다.
  - 최초 표본에서 한 건이라도 문제가 나오면 그 화행의 예비 사례를 전부 확인하고, 재발 시 해당 화행과 원자적 공개를 보류하는 규칙을 사전 등록했다.
  - 시스템 90%·95% 게이트는 연구자 Gold 30건 이상으로 이동하고, 외부 18건에서는 대표 일치율을 노출하지 않도록 readiness와 화면을 분리했다.
  - 504개 확인은 자동 결과 확인·경고 집중 검토로 다시 정의하고 경고 여부·확인 방식·시작시각·소요시간을 문항별로 저장하도록 했다.
- 검증 결과: typecheck, 전체 173개 테스트와 1,918-module production build가 통과했다. 세 migration을 원격 DB에 적용했고 새 함수의 DB lint 오류 0건과 원격 migration 최신 상태를 확인했다. localhost의 전체 현황·외부 표본·504 확인·최종 승인 화면에서 세 근거의 분리 문구를 확인했다.
- 다음 확인: 실제 Gold 모집단이 준비된 뒤 표본계획을 생성하고, 전문가 5건 예비검토로 45~60분 가정을 측정한다.
- 관련 Decision / Evidence: `DEC-20260815-09`, `EVD-20260815-09`
