...
## 배포 전 canary 결과 (2026-07-28)

프로덕션 `generate-scenario`를 바꾸지 않고 JWT 검증을 유지한 임시 canary
함수에서 DB 저장 없이 현재 코어 정본을 호출했다. 검사가 끝난 뒤 임시 함수는
Supabase에서 삭제하고 로컬 진입점도 제거했다.

| 대표 조건 | API·strict schema | 모델·hash | 사람 눈검사 |
|---|---|---|---|
| 반대 · 중급 · 번역 | 통과 | `gpt-4.1-mini` · `4c996a...71c0` | 통과 |
| 반대 · 중급 · 통역 | 통과 | `gpt-4.1-mini` · `4c996a...71c0` | 경계: 선행 발화의 시간 배분과 답변의 자료 해석이 섞여 동일 명제 대응이 흐려짐 |
| 불만 · 중급 · 번역 · IT 플랫폼 | 통과 | `gpt-4.1-mini` · `4c996a...71c0` | 통과 |

- 3건 모두 OpenAI의 strict JSON Schema 요청을 수락했고 필수 출력 구조와 새
  prompt hash를 반환했다. 폴백 모델은 사용되지 않았다.
- 응답 시간은 약 2.9~6.1초였다.
- 통역 반대 경계 셀의 교체 검증은 Supabase 앞단의 HTML 오류 응답으로 유효한
