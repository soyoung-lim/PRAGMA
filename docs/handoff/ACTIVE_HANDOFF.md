...
1. ~~B2 — R5 프롬프트 보강~~ **실행·측정 완료**(dev-log 참조). 엣지 배포 완료,
   미션 표본 9/9 생성, 완전 분리 억제 확인. **남은 것은 판정 하나** — A안(규칙 코드
   불변 + 계약 §8에 해석 명문화)을 확정할지. GPT 교차검증 권장 대상이며,
   **미션 조립 진행을 막지 않는다**(현행 규칙이 이미 그렇게 동작 중).
   함께 볼 것: 통역 셀 상황문에 서면 목적이 섞이는 패턴(코어 프롬프트 지침 사안).
