...
- 순서는 `Scale4 → Judge3+FixChoice4 → Reason3 → MultiJudge4 → DCT`다.
...
- Reason의 주원인이 하나이며 다른 두 이유가 동등하게 방어되지 않는가
- MultiJudge가 과소 1·적정 2·과잉 1이고 길이·형식이 정답 단서가 아닌가
...
`mission_v4_mpj4_dct1`은 2026-07-29 현재 저장소 구현과 로컬 검증 단계다.
