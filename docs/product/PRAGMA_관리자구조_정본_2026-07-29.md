...
- 순서는 `Scale4 → FixChoice(Judge3 제출 뒤 수정안 4개) → Reason(이유 3개) → MultiJudge(후보 5개) → DCT`다.
...
`mission_v4_mpj4_dct1_context_v4`와 `feedback_v1_feature_general_v2`는 2026-07-30 현재 저장소 구현과 로컬 검증 단계다.

- migration 원격 미적용
- Edge 원격 미배포
- 생성·저장·재조회 스모크 미실행
- v3 스모크 행을 v4로 변환하거나 reviewed로 승격하지 않음
- 로컬 프롬프트 스냅샷 12종은 `git_commit=0f3ccf6`, `git_dirty=false`로 재생성됨
...
- 현재 로컬 mission prompt는 `mission_v4_mpj4_dct1_context_v4`, runtime feedback prompt는 `feedback_v1_feature_general_v2`다. 저장소 정본과 실제 배포 active 여부는 별도로 표시한다.

## 8. target feature 관리

2026-07-30 현재 실행 정본은 코드 카탈로그다.
...
- 전체 테스트 146 pass / 생성형 golden 3 skip
- mission_v4·target feature·피드백 경계 관련 표적 테스트 4개 파일, 22개 pass
- 프롬프트 스냅샷 12종, 코어 표면 해시 `4c996a00259cf54dcc23b03d0998f7afd3926a95c284ed23719910ebb1d871c0`
