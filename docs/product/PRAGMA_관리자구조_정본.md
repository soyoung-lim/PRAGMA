...
| AI 독립 검토 | `/admin/cross-vendor` | 계약 §5.4 AI 모델 간 독립 검토의 **열람 전용** 화면 (실행 트리거 없음, 2026-08-07 신설) |
...
- 화면명은 `HSK 3.0 어휘 기준·점검`으로 한다. `0 · 설계 기준` 메뉴의 정보 순서는
  `탑재 데이터와 PRAGMA 수준 대응 → 실제 콘텐츠 점검 결과`로 한다. 운영 결과 영역 안에서는
  절차 설명보다 `전수 점검 수 → HSK 참고 일치 → 교수자 확인` 결과를 우선한다. 일치율은 최근
  콘텐츠 한 건의 HSK 참고 대조 결과로 명시하고, 평가 점수·자동 통과율·전체 콘텐츠 품질로
  표현하지 않는다.
- LOCK 전 층화 시험은 최근 사례 카드와 분리한 전폭 상태 줄에서 `12개 조건 · 시험 콘텐츠 30건 ·
  점검 결과 저장 30/30`이라는 장치 작동 증거로만 표시한다. 30건 평균 일치율은 폐기 예정 테스트
  콘텐츠의 품질 통계로 오독될 수 있으므로 화면에 표시하지 않는다. `PRAGMA 수준 선택 → HSK 누적
  범위 대조 → 확인 항목을 교수자에게 전달` 절차는 최근 결과 아래 한 줄로 압축한다.
...
- 최근 결과는 `49개 중 35개를 HSK 참고 범위에서 확인`이라는 분모가 있는 문장으로 시작한다.
  71%는 독립 지표가 아니라 `35/49`의 보조 설명으로만 표시하고, 남은 14개는 자동 오류가 아닌
  교수자 확인 항목임을 바로 연결한다.
