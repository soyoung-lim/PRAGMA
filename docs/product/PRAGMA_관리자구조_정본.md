...
| HSK 3.0 데이터셋 | `/admin/corpus` | 방향별 중국어 어휘 점검, 수준별 참고 범위·예시, 최근 점검 기록과 주제·출처 참고자료 열람 |
...
사이드바의 `3 · 수업·학습 운영`은 `AI 15주 교과목 설계`·`학습자 관리`·`수행·의사결정 기록`을
...
- 화면의 첫 관계는 `PRAGMA 수준 → HSK 어휘 참고 범위 → 교수자 판단`이다. PRAGMA가 학습
  수준을 설계하고 HSK는 생성된 중국어의 어휘 난이도를 대조하는 외부 참고 기준이며, 목록 밖
  후보의 최종 판단은 교수자가 한다. 세 요소를 독립 KPI처럼 표시하지 않는다.
- 한→중은 생성된 중국어 예시·선택지·참고안을, 중→한은 학습자에게 제시할 중국어 원문을
  수준별 누적 어휘 범위와 대조한다. 번역·통역 모드는 이 화면에서 별도 적용축으로 표시하지 않는다.
- 입문·중급·고급은 하나의 공유 수준 프레임 안에서 HSK 1–4·1–5·1–6급 누적 범위,
  2,000·3,600·5,400개, 공식 어휘 예시와 함께 보여 준다. PRAGMA 수준명이 가장 먼저 읽히며,
  대표 어휘 펼침은 수준별로 독립 동작한다. 같은 누적 수치를 별도 카드로 반복하지 않는다.
- 중국어 생성→HSK 참고 범위 대조→목록 밖 후보 표시→교수자 검수의 흐름과 최근 저장 결과의
  실제 점검 기록을 평이한 언어로 표시한다. 목록 밖 단어는 자동 오류·차단이 아니라 사람 검토 후보다.
  최근 기록이 없을 때는 가짜 숫자 대신 향후 표시 필드와 통합 검수 연결을 보여 준다.
- 공식 HSK 주제 427행은 현재 PRAGMA의 P·D·R·domain·industry·theme·topic 생성 조건을
  대체하거나 보강하지 않는다. 접힌 `참고자료`로 보관하며, 향후 PRAGMA 시나리오의 주제 범위를
  공신력 있는 외부 기준과 비교하는 보조 검증에 사용할 수 있다.
- 공식 시험대강 title·publisher·발행/시행 version·URL·SHA-256과 내부 데이터 판본은 기본
  작업 흐름 아래의 출처 영역에서 확인한다. 같은 영역에 HSK가 인증 기준이 아니며 범위 밖 단어가
  자동 탈락 조건이 아니라는 데이터 지위를 한 번만 구조적으로 설명한다. 상세 데이터 층과 등급별
  신규 도입 수치는 최하단 보조 아코디언에서만 확인한다.
