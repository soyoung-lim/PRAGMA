...
섞지 않고 소량 검수 → 전체 재생성을 반복한다. 현재 작업 단위는 통역 3자 역할 분리와
학생용 평가 기준 비노출을 반영한 `pragma_content_candidate_20260805_01`이다.
`pragma_content_candidate_20260804_01`~`_04`는 과거 후보이며 새 후보 산출물과 같은 검수
묶음으로 취급하지 않는다.
