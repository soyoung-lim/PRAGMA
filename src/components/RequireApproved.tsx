...
type Props = {
  ...
  /** 로그인 상태와 무관하게 명시적인 DEV mission_v4/v5/v6 검토 링크만 연다. */
...
export const RequireApproved = ({ children, allowDevMissionPreview = false }: Props) => {
  ...
  const isDevMissionPreview =
    ...
    (previewVersion === "v4" || previewVersion === "v5" || previewVersion === "v6");

  // 검토 URL은 새 탭에서 열리므로 sessionStorage 기반 DEV stub을 공유하지 못한다.
  // 실제 시나리오 경로와 production에는 적용하지 않고, 명시적 v4/v5/v6 preview만 우회한다.
