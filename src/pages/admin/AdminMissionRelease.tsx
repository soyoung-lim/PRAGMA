...
const PREVIEW_REGRESSION: RegressionRow = { id: "40000000-0000-4000-8000-000000000051", realization_pack_id: "pragma_ko_zh_core", realization_pack_version: "1.2.0", gate_status: "pass", evaluator_version: "preview-evaluator-v1", report: { mode: "researcher_gold_operational_gate", reference_case_count: FINAL_GOLD_POPULATION_COUNT, gate_status: "pass" }, evaluation_purpose: "operational_gate_check", is_quality_measurement: false, source_authority: "researcher_calibration", interpretation_note_ko: `연구 책임자가 확정한 기준답안 ${FINAL_GOLD_POPULATION_COUNT}건으로 품질 점검 자동화의 작동 조건만 확인합니다. 외부 전문가 ${EXTERNAL_GOLD_SAMPLE_COUNT}건의 내용타당성 확인과 별개입니다.`, created_at: "2026-08-15T00:20:00.000Z" };
  ...
  return <AdminShell title="4단계 · 교수자 검수·승인" description={`판정 기준 ${FINAL_GOLD_POPULATION_COUNT}개, 외부 전문가 확인 ${EXTERNAL_GOLD_SAMPLE_COUNT}개, AI 생성 학습 콘텐츠 504개의 자동 점검 결과를 구분해 확인한 뒤 교수자가 학습자 공개를 승인합니다.`}>
    <div className="space-y-5">
      ...
      <section className="grid gap-3 md:grid-cols-3">
        <div className="rounded-xl border bg-white p-4"><p className="text-xs font-semibold text-slate-500">시스템 판단 게이트</p><p className="mt-2 text-sm leading-6">연구 책임자가 확정한 기준답안 {FINAL_GOLD_POPULATION_COUNT}개로 자동 판정 장치의 작동 조건을 확인합니다.</p></div>
        <div className="rounded-xl border bg-white p-4"><p className="text-xs font-semibold text-slate-500">외부 내용타당성 확인</p><p className="mt-2 text-sm leading-6">전문가 2인이 사전 추출한 {EXTERNAL_GOLD_SAMPLE_COUNT}개를 독립 확인합니다. 504개의 전문가 검증으로 표현하지 않습니다.</p></div>
      ...
      <section className="rounded-xl border bg-white p-5">
        <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="flex items-center gap-2 font-semibold"><PlayCircle className="h-5 w-5" />1. 기준답안 {FINAL_GOLD_POPULATION_COUNT}개 기반 시스템 판단 게이트</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">연구 책임자가 확정한 기준답안 모집단 전체로 품질 점검 자동화가 정해진 조건대로 작동하는지 확인합니다. 외부 전문가 {EXTERNAL_GOLD_SAMPLE_COUNT}개 내용타당성 확인과는 별도 단계입니다.</p></div><Badge variant="outline">준비된 기준답안 {preview ? FINAL_GOLD_POPULATION_COUNT : goldForPack.length}/{FINAL_GOLD_POPULATION_COUNT}</Badge></div>
        <p className="mt-4 rounded-lg border border-rose-200 bg-rose-50 p-4 text-sm leading-6 text-rose-950"><strong>해석 경계:</strong> 90%·95%는 기준답안 {FINAL_GOLD_POPULATION_COUNT}개에서 사용하는 내부 운영 통과 조건입니다. 전체 시스템 정확도나 일반화된 품질 측정치로 보고하지 않습니다. 외부 전문가 {EXTERNAL_GOLD_SAMPLE_COUNT}개에서는 일치율·카파를 대표 결과로 제시하지 않습니다.</p>
        {goldForPack.length < FINAL_GOLD_POPULATION_COUNT && <p className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">1단계에서 9개 화행을 화행별 {FINAL_GOLD_CASES_PER_SPEECH_ACT}개씩 포함한 기준답안 {FINAL_GOLD_POPULATION_COUNT}개를 먼저 확정하세요. 기존 3화행 Seed 30개만으로는 이 gate를 열 수 없습니다.</p>}
        ...
        <Button className="mt-3" onClick={recordRegression} disabled={preview || saving || goldForPack.length < FINAL_GOLD_POPULATION_COUNT || !evaluatorVersion.trim() || !promptHash.trim()}><Save className="mr-1 h-4 w-4" />시스템 판단 게이트 기록 저장</Button>
      </section>
