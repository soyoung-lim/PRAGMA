...
const ExpertGoldReviewQueue = ({ preview = false }: { preview?: boolean }) => {
  ...
  const submit = async () => {
    ...
    try {
      ...
      setMessage("독립 Gold 검토를 append-only로 제출했습니다.");
  ...
  const submitSignoff = async () => {
    ...
    if (error) setMessage(error.message);
    else { setSignoffs((current) => [data as SignoffRow, ...current]); setMessage("resolution 서명을 저장했습니다."); }
    setSaving(false);
  };

  return (
    <div className="min-h-screen bg-[#f7f5ef] text-slate-900">
      <header className="bg-[#15202B] px-5 py-4"><div className="mx-auto flex max-w-5xl items-center justify-between"><HomeBrand /><Link to="/expert/reviews" className="text-sm text-slate-300 hover:text-white">문항 lineage 검토 →</Link></div></header>
      <main className="mx-auto max-w-5xl space-y-5 px-4 py-7 sm:px-6">
        <section className="rounded-2xl border bg-white p-5 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-amber-700">External Gold Review</p><h1 className="mt-1 text-2xl font-semibold">Gold 외부 전문가 독립 검토</h1><p className="mt-2 max-w-3xl text-sm text-slate-600">연구자의 기대 대역·해설·rule/risk 참조는 이 snapshot에 포함되지 않습니다. 제출 전에는 다른 전문가의 답도 보이지 않습니다.</p></div>
            <Badge className="gap-1 bg-slate-900 text-white"><EyeOff className="h-3.5 w-3.5" /> blind v1</Badge>
          </div>
        </section>

        <section className="rounded-2xl border bg-white p-5">
          <label className="text-sm font-medium">내 배정</label>
          <Select value={selectedId} onValueChange={(value) => { setSelectedId(value); setCandidates(emptyCandidates()); setContext({ scenario_valid: false, pdr_valid: false, semantic_invariant_valid: false }); setOverall(""); setRationale(""); }}>
            ...
            <SelectContent>{assignments.map((assignment) => <SelectItem key={assignment.id} value={assignment.id}>{assignment.blind_case_snapshot.case_id} · round {assignment.review_round}</SelectItem>)}</SelectContent>
          </Select>
        </section>

        {selected && <>
          <section className="rounded-2xl border bg-white p-5">
            <div className="flex flex-wrap gap-2"><Badge variant="outline">{selected.blind_case_snapshot.speech_act}</Badge><Badge variant="outline">P {selected.blind_case_snapshot.pdr.power}</Badge><Badge variant="outline">D {selected.blind_case_snapshot.pdr.distance}</Badge><Badge variant="outline">R {selected.blind_case_snapshot.pdr.burden}</Badge></div>
            <h2 className="mt-4 font-semibold">상황과 의미 불변항</h2>
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-3">{([
              ["scenario_valid", "상황이 자연스럽다"], ["pdr_valid", "P/D/R 부호가 타당하다"], ["semantic_invariant_valid", "의미 불변항이 타당하다"],
          ...
          <section className="space-y-3">{selected.blind_case_snapshot.candidates.map((candidate) => {
            const draft = candidates[candidate.candidate_id];
            return <article key={candidate.candidate_id} className="rounded-2xl border bg-white p-5"><div className="flex gap-3"><Badge>{candidate.candidate_id}</Badge><p className="break-words text-lg leading-relaxed">{candidate.text_zh}</p></div><div className="mt-4 grid gap-3 md:grid-cols-2"><Select value={draft.assessed_band_code} onValueChange={(value) => setCandidates((current) => ({ ...current, [candidate.candidate_id]: { ...current[candidate.candidate_id], assessed_band_code: value } }))} disabled={Boolean(storedReview)}><SelectTrigger><SelectValue placeholder="독립 대역 판정" /></SelectTrigger><SelectContent>{BANDS.map(([value, label]) => <SelectItem key={value} value={value}>{label}</SelectItem>)}</SelectContent></Select><Select value={draft.semantic_fidelity} onValueChange={(value: "pass" | "fail") => setCandidates((current) => ({ ...current, [candidate.candidate_id]: { ...current[candidate.candidate_id], semantic_fidelity: value } }))} disabled={Boolean(storedReview)}><SelectTrigger><SelectValue placeholder="의미 충실성" /></SelectTrigger><SelectContent><SelectItem value="pass">의미 유지</SelectItem><SelectItem value="fail">의미 변형/손실</SelectItem></SelectContent></Select></div><Textarea className="mt-3" value={draft.rationale_ko} onChange={(event) => setCandidates((current) => ({ ...current, [candidate.candidate_id]: { ...current[candidate.candidate_id], rationale_ko: event.target.value } }))} placeholder="판정 근거" disabled={Boolean(storedReview)} /></article>;
          })}</section>

          <section className="rounded-2xl border bg-white p-5">
            <div className="grid gap-3 md:grid-cols-2"><Select value={overall} onValueChange={(value: "approve" | "revise" | "reject") => setOverall(value)} disabled={Boolean(storedReview)}><SelectTrigger><SelectValue placeholder="종합 판정" /></SelectTrigger><SelectContent><SelectItem value="approve">사용 가능</SelectItem><SelectItem value="revise">수정 필요</SelectItem><SelectItem value="reject">기각</SelectItem></SelectContent></Select><Textarea value={rationale} onChange={(event) => setRationale(event.target.value)} placeholder="종합 근거" disabled={Boolean(storedReview)} /></div>
            ...
            <Button className="mt-5 gap-2" disabled={!complete || saving || preview || Boolean(storedReview)} onClick={submit}><Save className="h-4 w-4" />{storedReview ? "제출 완료" : "독립 검토 제출"}</Button>
          </section>

          {visibleResolution && <section className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5"><div className="flex items-center gap-2"><ShieldCheck className="h-5 w-5 text-emerald-700" /><h2 className="font-semibold">해결본 #{visibleResolution.id.slice(0, 8)}</h2></div><p className="mt-2 text-sm">{visibleResolution.final_status} · {visibleResolution.resolution_method}</p><p className="mt-1 text-sm text-slate-700">{visibleResolution.rationale_ko}</p>{visibleResolution.resolution_method === "consensus_after_discussion" && !signoffs.some((item) => item.resolution_id === visibleResolution.id) && <div className="mt-4 grid gap-3 md:grid-cols-[180px_1fr_auto]"><Select value={signoffDecision} onValueChange={(value: "agree" | "disagree") => setSignoffDecision(value)}><SelectTrigger><SelectValue placeholder="합의 확인" /></SelectTrigger><SelectContent><SelectItem value="agree">동의</SelectItem><SelectItem value="disagree">동의하지 않음</SelectItem></SelectContent></Select><Textarea value={signoffRationale} onChange={(event) => setSignoffRationale(event.target.value)} placeholder="서명 근거" /><Button onClick={submitSignoff} disabled={saving || preview || !signoffDecision || !signoffRationale.trim()}>서명</Button></div>}</section>}
        </>}
        {message && <p className="rounded-xl border bg-white p-3 text-sm">{message}</p>}
        {preview && <p className="text-xs text-slate-500">개발 preview에서는 DB 쓰기가 잠겨 있습니다.</p>}
      </main>
    </div>
  );
};
