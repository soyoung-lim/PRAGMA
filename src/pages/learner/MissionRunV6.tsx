...
/** 학생의 선택 행동에는 짧은 대역명만 보여 주고 카탈로그의 괄호 설명은 내부에 둔다. */
function learnerBandLabel(label: string): string {
...
export function MissionV6Runner({
  mission,
  isSample,
  startAtPart2 = false,
  headerRight,
      ...
      <div className="pb-24">
        ...
        {phase === "revise" && (
          <div className="space-y-3">
            ...
            <div className={card}>
              <div className="text-[12.5px] font-semibold">어떻게 제출할까요?</div>
              ...
              {revisionDecision === "keep" && revisionRequired && (
                <Textarea
                  ...
                  placeholder="그대로 제출하려는 이유를 간단히 써 주세요. (이견을 이미 남겼다면 생략 가능)"
                />
              )}
              {revisionDecision === "revise" && !revisionCanSubmit && (
                <p className="mt-2 text-[11.5px] text-[#8A5A17]">
                  글자 간격이나 문장부호만 바꾸지 말고, 피드백을 반영해 문장을 고쳐 주세요.
                </p>
              )}
            </div>
            <Button
...
function judgmentResponseCountFor(itemType: string): number {
  return ({ scale4: 1, fix_choice: 3, fix_review: 2, multi_judge: 3 } as Record<string, number>)[itemType] ?? 1;
}
...
function MpjStageCurrent({
  ...
  const classificationOptions = featureBands.map((band) => ({
    code: band.code,
    label: learnerBandLabel(band.label_ko),
  }));
              ...
              >
                ...
                {answered && (
                  <div className="mt-2 text-[12.5px] text-muted-foreground">
                    이 상황에서의 판단 · {c.accepted_band_codes.map((code) => bandLabel(feature, code)).join(" / ")} · {c.note_ko}
                  </div>
                )}
              </li>
            ))}
